...
opioid_er_data = opioid_er_data %>% 
  ...
  rename(county = patient_county_name) %>% print()
...
patients = opioid_total_data %>% group_by(year, county) %>% summarize(opioids = sum(overall_opioid)) %>% filter(county != 'Statewide Total') %>% mutate(county = toupper(county)) %>% mutate(county = ifelse(county == 'RICHMOND (STATEN ISLAND)', 'RICHMOND', county)) 
...
time_plot = ggplot(data = prod_county, aes(x = year, y = numpills, group = county)) +
  ...
  labs(title = 'Number of Prescriptions per County', x = 'Year', y = 'Number of Perscriptions (aggregate)') +
  theme_bw()+
