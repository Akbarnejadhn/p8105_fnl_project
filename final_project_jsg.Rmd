...
```{r}
left_join(death_sales_dat, death_int_slope_df) %>% 
  ggplot(aes(x = year, y = opioid_poisoning_deaths/population*10000, group = county, color = .cluster)) + 
  geom_point() + 
  geom_path() +
...
```{r}
left_join(er_sales_dat, er_int_slope_df) %>% 
  ggplot(aes(x = year, y = er_opioid/population*10000, group = county, color = .cluster)) + 
  geom_point() + 
  geom_path() +
  scale_x_continuous(breaks = c(0, 1, 2), labels = c("2010", "2011", "2012")) +
  theme(legend.position = "none")
...
join_dat <- join_df %>%
  ...
  mutate(year = factor(year),
         region = ifelse(county %in% c("New York", "Kings", "Bronx", "Queens", "Staten Island"), "NYC", "ROS")) %>% 
  select(-county)
