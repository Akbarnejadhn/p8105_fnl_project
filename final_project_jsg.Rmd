...
pills_plot <- left_join(int_slope_dat, int_slope_df) %>% 
  group_by(county) %>% 
  ggplot(aes(x = year, y = ppp, color = .cluster)) + 
  geom_point() + 
  geom_path() +
...
deaths_plot <- left_join(death_sales_dat, death_int_slope_df) %>% 
  group_by(county) %>% 
  ggplot(aes(x = year, y = opioid_poisoning_deaths/population*10000, color = .cluster)) + 
  geom_point() + 
  geom_path() +
...
er_plot <- left_join(er_sales_dat, er_int_slope_df) %>% 
  group_by(county) %>% 
  ggplot(aes(x = year, y = er_opioid/population*10000, color = .cluster)) + 
  geom_point() +
  geom_path() +
  scale_x_continuous(breaks = c(0, 1, 2), labels = c("2010", "2011", "2012")) +
  theme(legend.position = "none")
